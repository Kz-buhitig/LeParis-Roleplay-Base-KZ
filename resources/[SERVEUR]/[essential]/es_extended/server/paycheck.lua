		...
		for i=1, #xPlayers, 1 do
			...
			if salary2 > 0 then
				...
				xPlayer.addAccountMoney('bank', salary2)
					...
					TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck2'), _U('received_help2', salary2), 'CHAR_JOE', 9)
						...
						if society ~= nil then -- verified society
							...
							TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function (account)
								...
								if account.money >= salary2 then -- does the society money to pay its employees?
									...
									TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck2'), _U('received_salary2', salary2), 'CHAR_JOE', 9)

								else

									TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), '', _U('company_nomoney'), 'CHAR_JOE', 1)
