...
Citizen.CreateThread(function()
	while true do
		Wait(30000)
		TriggerServerEvent("godzilla:timer")
	end
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if speed > 9.0 and not veh and (para == -1 or para == 0) and not flyveh and not fall and not parafall and not rag then
			...
			--TriggerServerEvent("Godzilla:SpeedFlag", rounds, roundm) -- send alert along with the rounded speed and how much faster they are
		end
	end
end)
	...
	while true do
		...
		if not IsPlayerDead(PlayerId()) then
			if PlayerPedId() == curPed and GetEntityHealth(curPed) == curHealth and GetEntityHealth(curPed) ~= 0 then
				TriggerServerEvent("Godzilla:HealthFlag", false, curHealth-2, GetEntityHealth( curPed ),curWait )
			elseif GetEntityHealth(curPed) == curHealth-2 then
				SetEntityHealth(curPed, GetEntityHealth(curPed)+2)
			end
		end
		if GetEntityHealth(curPed) > 400 then
			TriggerServerEvent("Godzilla:HealthFlag", false, GetEntityHealth( curPed )-200, GetEntityHealth( curPed ),curWait )
		end
		...
		if GetPlayerInvincible( PlayerId() ) then -- if the player is invincible, flag him as a cheater and then disable their invincibility
			TriggerServerEvent("Godzilla:HealthFlag", true, curHealth-2, GetEntityHealth( curPed ),curWait )
	...
	while true do
		...
		for _,theWeapon in ipairs(BlacklistedWeapons) do
			...
			if HasPedGotWeapon(PlayerPedId(),GetHashKey(theWeapon),false) == 1 then
					TriggerServerEvent("Godzilla:WeaponFlag", theWeapon)
					break
			end
		end
	end
...
RegisterNetEvent("Godzilla:RemoveInventoryWeapons")
AddEventHandler('Godzilla:RemoveInventoryWeapons', function()
	...
	while true do
		...
		if IsPedJumping(PlayerPedId()) then
			...
			if jumplength > 250 then
				TriggerServerEvent("Godzilla:JumpFlag", jumplength )
			end
		end
	end
end)
