...
function isPlayerBoss2(playerId, job2)
	...
	if xPlayer.job2.name == job2 and xPlayer.job2.grade_name == 'boss' then
		...
		print(('esx_society: %s attempted open a society boss menu2!'):format(xPlayer.identifier))
		return false
	end
end
...
ESX.RegisterServerCallback('esx_society:getEmployees2', function(source, cb, society)
	if Config.EnableESXIdentity then
		...
		MySQL.Async.fetchAll('SELECT identifier, job2, job2_grade FROM users WHERE job2 = @job2 ORDER BY job2_grade DESC', {
			['@job2'] = society
		}, function (result)
			local employees = {}
...
ESX.RegisterServerCallback('esx_society:setJobSalary2', function(source, cb, job, grade, salary)
	...
	if xPlayer.job2.name == job and xPlayer.job2.grade_name == 'boss' then
		if salary <= Config.MaxSalary then
			MySQL.Async.execute('UPDATE job2_grades SET salary = @salary WHERE job2_name = @job2_name AND grade2 = @grade2', {
				['@salary']   = salary,
				['@job2_name'] = job2,
				['@grade']    = grade2
			}, function(rowsChanged)
				Jobs[job].grades[tostring(grade2)].salary = salary
				local xPlayers = ESX.GetPlayers()
