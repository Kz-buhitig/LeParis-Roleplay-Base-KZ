appartements = {
	{ 
		--ext = {name = "1 - Hall principale", x = -1096.3503417969, y = -850.11041259766, z = 19.001203536987-1.001, h = 39.06},
		appts = {
			{name = "-1 - Céllules/Garage", x = -1096.4840087891, y = -850.0830078125, z = 4.8841795921326-1.001, h = 39.06},
			{name = "-2 - Salle des saisies/Laboratoire", x = -1096.3061523438, y = -850.20513916016, z = 10.276628494263-1.001, h = 39.06},
			{name = "-3 - Garage/Armurie", x = -1096.3273925781, y = -850.12567138672, z = 13.687370300293-1.001, h = 39.06},
			{name = "1 - Hall principale", x = -1096.3503417969, y = -850.11041259766, z = 19.001203536987-1.001, h = 39.06},
			{name = "2 - Cafèteria", x = -1096.3275146484, y = -850.13134765625, z = 23.038642883301-1.001, h = 39.06},
			{name = "3 - Vestiaire/Armurie/Salle de sport", x = -1096.3118896484, y = -850.04034423828, z = 26.827590942383-1.001, h = 39.06},
			{name = "4 - Centre des Opérations/Bureaux HG", x = -1096.2993164063, y = -850.07202148438, z = 30.757154464722-1.001, h = 39.06},
			{name = "5 - Bureau du Capitaine", x = -1096.3863525391, y = -849.93432617188, z = 34.360733032227-1.001, h = 39.06},
			{name = "6 - Toit (Hélico)", x = -1096.4581298828, y = -849.95629882813, z = 38.243228912354-1.001, h = 39.06}
		}
	}
}
		...
		for id=1, #appartements do
			...
			if dist_ext < 15 then-- Test de distance pour afficher le marqueur. (Qu'on ne le voit pas de loin)
				if dist_ext <= 0.9 then -- Distance Interne Marqueur pour déclencher la téléportation
					ShowHelp("Appuyé sur ~INPUT_VEH_HORN~ pour entrer.")
			...
			for appt=1, #appts do
				...
				if dist_in < 8 then -- Test de distance pour afficher le marqueur. (Qu'on ne le voit pas de loin)
					if dist_in <= 1.3 then -- Distance Interne Marqueur pour déclencher la téléportation
						ShowHelp("Appuyé sur ~INPUT_VEH_HORN~ pour allé à l'étage séléctioné")
						if IsControlJustReleased(0, 86) then
							ShowMenu(id, appartements[id].appts)
						end
					end
