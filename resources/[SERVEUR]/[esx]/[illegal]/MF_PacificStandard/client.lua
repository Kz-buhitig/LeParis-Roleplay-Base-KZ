    ...
    elseif act == "LootA" or act == "LootB" or act == "LootC" or act == "LootD" or act == "LootE" or act == "LootF" or act == "LootG" or act == "LootH" then
        ESX.TriggerServerCallback('MF_PacificStandard:GetOxyCount', function(count)
            if count and count > 0 then
                ...
                ESX.ShowNotification("Vous avez besoin d'une torche plasma pour ouvrir.")
                self.DoingAction = false
            end
        end)
...
function MFP:HandleLootCash()
    ...
    if (plySkin["bags_1"] ~= 0 or plySkin["bags_2"] ~= 0) then
        Citizen.CreateThread(function(...)
            ...
            exports['progressBars']:startUI(self.InteractTimer * 1000, "Ramasse les billets")
        ...
        ESX.ShowNotification("Vous avez besoin d'un sac pour transporter tout cet argent.")
        self.DoingAction = false
    end
end
...
function MFP:NotifyCops()
  ESX.ShowNotification("Quelqu\'un est en train de voler la PACIFIC BANK")  
  Citizen.CreateThread(function(...)
