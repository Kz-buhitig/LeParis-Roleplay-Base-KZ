...
function openInventory()
    ...
    ESX.TriggerServerCallback('esx_illegals:getInventory', function(result)
        ...
        table.insert(elements, {label = "Dépot", value = "deposit"})
            ...
            local invitem = result.items[i]
      		...
      		for k,v in ipairs(Config.Items) do
      			...
      			if invitem.count > 0 and invitem.name == v then
                	...
                	table.insert(elements2, { label = invitem.label .. ' | ' .. invitem.count .. ' dans votre inventaire', type = "item_standard", count = invitem.count, value = invitem.name})
                ...
                if data.current.value == "deposit" then
					...
					function(data, menu)
						...
						if invcount ~= nil and amount > invcount and hasItems then
                                        ...
                                        ESX.ShowNotification('~r~Tu ne peux pas déposé plus que tu as.')
