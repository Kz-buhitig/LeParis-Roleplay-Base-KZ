          ...
          if Config.MaxInService == -1 then
            ESX.Game.SpawnVehicle(model, {
			  ...
			  SetVehicleNumberPlateText(vehicle," BLOODS  “ .. numberplate .. ”")
                          SetVehicleCustomPrimaryColour(vehicle, 133, 15, 17)
                          SetVehicleCustomSecondaryColour(vehicle, 79, 4, 10)
            end)
          else
...
local blips = {
  {title="~r~Zone Bloods", colour=76, id=84, x = 104.67, y = -1938.46, z = 20.8}
  }
  Citizen.CreateThread(function()
		Citizen.Wait(0)
local bool = true
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job2 ~= nil and PlayerData.job2.name == 'bloods' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'menu_boss_actions' then
          ESX.UI.Menu.CloseAll()
          TriggerEvent('esx_society:openBossMenu2', 'bloods', function(data, menu)
            menu.close()
            CurrentAction     = 'menu_boss_actions'
            CurrentActionMsg  = _U('open_bossmenu')
            CurrentActionData = {}
