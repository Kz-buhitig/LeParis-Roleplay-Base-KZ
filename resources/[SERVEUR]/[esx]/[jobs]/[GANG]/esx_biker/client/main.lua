        ...
        if not DoesEntityExist(vehicle) then
          ...
          if Config.MaxInService == -1 then
            ESX.Game.SpawnVehicle(model, {
			  ...
			  SetVehicleNumberPlateText(vehicle," LOSTMC  “ .. numberplate .. ”")
                          SetVehicleCustomPrimaryColour(vehicle, 47, 43, 43)
                          SetVehicleCustomSecondaryColour(vehicle, 94, 89, 89)
            end)
          else
...
local blips = {
  {title="~u~Zone Biker", colour=40, id=84, x = 956.103, y = -117.974, z = 75.004}
  }
  Citizen.CreateThread(function()
		Citizen.Wait(0)
local bool = true
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job2 ~= nil and PlayerData.job2.name == 'biker' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'menu_boss_actions' then
          ESX.UI.Menu.CloseAll()
          TriggerEvent('esx_society:openBossMenu2', 'biker', function(data, menu)
            menu.close()
            CurrentAction     = 'menu_boss_actions'
            CurrentActionMsg  = _U('open_bossmenu')
            CurrentActionData = {}
