	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			if Config.Uniforms.male then
				...
				ESX.ShowAdvancedNotification(_U('notif_title_delivery'), '', _U('info'), 'CHAR_ANDREAS', 1 )
			else
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, 56) and IsJobTrucker() and isInService then -- Touche F6 si il est bien dans le job & en service
        	Livraison()
        end
