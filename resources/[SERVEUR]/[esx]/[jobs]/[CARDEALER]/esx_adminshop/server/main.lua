...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('esx_adminshop: ^1WARNING^7 plate character count reached, %s/8 characters.'):format(char))
	end
end)
...
MySQL.ready(function()
	Categories     = MySQL.Sync.fetchAll('SELECT * FROM vs_vip_categories')
	local vehicles = MySQL.Sync.fetchAll('SELECT * FROM vs_vips')
	...
	TriggerClientEvent('esx_adminshop:sendCategories', -1, Categories)
	TriggerClientEvent('esx_adminshop:sendVehicles', -1, Vehicles)
end)

RegisterServerEvent('esx_adminshop:setVehicleOwned')
AddEventHandler('esx_adminshop:setVehicleOwned', function (vehicleProps,vehicleData)
	...
	if found then
		...
		MySQL.Async.execute('INSERT INTO owned_vehicles (vehicle, owner, plate, vehiclename, originalmodel) VALUES (@vehicle, @owner, @plate, @vehiclename, @originalmodel)',
		{
		  ['@vehicle'] = json.encode(vehicleProps),
		  ['@owner']   = xPlayer.identifier,
		  ['@plate'] = vehicleProps.plate,
		  ['@vehiclename'] = vehicleData.model,
		  ['@originalmodel'] = vehicleData.model
		}, function (rowsChanged)
			TriggerClientEvent('esx:showNotification', _source, _U('vehicle_belongs', vehicleProps.plate))
		end)
	else
		TriggerClientEvent('esx:deleteVehicle', _source)
		print(('esx_adminshop: %s attempted to inject vehicle!'):format(xPlayer.identifier))
	end
end)

RegisterServerEvent('esx_adminshop:setVehicleOwnedPlayerId')
AddEventHandler('esx_adminshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps,vehicleData)
	local _source = source
	...
	if found and xPlayerSource.job.name == 'admin' then
		MySQL.Async.execute('INSERT INTO owned_vehicles (vehicle, owner, plate, vehiclename, originalmodel) VALUES (@vehicle, @owner, @plate, @vehiclename, @originalmodel)',
		{
		  ['@vehicle'] = json.encode(vehicleProps),
		  ['@owner']   = xPlayer.identifier,
...
RegisterServerEvent('esx_adminshop:setVehicleOwnedSociety')
AddEventHandler('esx_adminshop:setVehicleOwnedSociety', function (society, vehicleProps)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_adminshop:sellVehicle')
AddEventHandler('esx_adminshop:sellVehicle', function (vehicle)
...
RegisterServerEvent('esx_adminshop:addToList')
AddEventHandler('esx_adminshop:addToList', function(target, model, plate)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(target)
	local dateNow = os.date('%Y-%m-%d %H:%M')

	if xPlayer.job.name ~= 'admin' then
		print(('esx_adminshop: %s attempted to add a sold vehicle to list!'):format(xPlayer.identifier))
		return
	end
...
ESX.RegisterServerCallback('esx_adminshop:getSoldVehicles', function (source, cb)
...
RegisterServerEvent('esx_adminshop:rentVehicle')
AddEventHandler('esx_adminshop:rentVehicle', function (vehicle, plate, playerName, basePrice, rentPrice, target)
...
RegisterServerEvent('esx_adminshop:getStockItem')
AddEventHandler('esx_adminshop:getStockItem', function (itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_adminshop:putStockItems')
AddEventHandler('esx_adminshop:putStockItems', function (itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('esx_adminshop:getCategories', function (source, cb)
	cb(Categories)
end)

ESX.RegisterServerCallback('esx_adminshop:getVehicles', function (source, cb)
	cb(Vehicles)
end)

RegisterCommand("don", function(source, args, rawCommand)
    if IsPlayerAceAllowed(source, "fax.Admin") then
        TriggerClientEvent("don:open", source)
    else
        TriggerClientEvent("chatMessage", source, "^1Insufficient Permissions.")
    end
end)

ESX.RegisterServerCallback('esx_adminshop:buyVehicle', function (source, cb, vehicleModel)
...
ESX.RegisterServerCallback('esx_adminshop:buyVehicleSociety', function (source, cb, society, vehicleModel)
	local vehicleData = nil
...
ESX.RegisterServerCallback('esx_adminshop:getCommercialVehicles', function (source, cb)
...
RegisterServerEvent('esx_adminshop:returnProvider')
AddEventHandler('esx_adminshop:returnProvider', function(vehicleModel)
	local _source = source
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			print(('esx_adminshop: %s attempted selling an invalid vehicle!'):format(GetPlayerIdentifiers(_source)[1]))
		end

	end)
end)

ESX.RegisterServerCallback('esx_adminshop:getRentedVehicles', function (source, cb)
...
ESX.RegisterServerCallback('esx_adminshop:giveBackVehicle', function (source, cb, plate)
...
ESX.RegisterServerCallback('esx_adminshop:resellVehicle', function (source, cb, plate, model)
	local resellPrice = 0
	...
	if resellPrice == 0 then
		print(('esx_adminshop: %s attempted to sell an unknown vehicle!'):format(GetPlayerIdentifiers(source)[1]))
		cb(false)
	else
		MySQL.Async.fetchAll('SELECT * FROM rented_vehicles WHERE plate = @plate', {
			['@plate'] = plate
					...
					if result[1] then -- does the owner match?
						...
						if vehicle.model == model then
							if vehicle.plate == plate then
								...
								print(('esx_adminshop: %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						else
							print(('esx_adminshop: %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
							cb(false)
						end
					else
...
ESX.RegisterServerCallback('esx_adminshop:getStockItems', function (source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('esx_adminshop:getPlayerInventory', function (source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_adminshop:isPlateTaken', function (source, cb, plate)
...
ESX.RegisterServerCallback('esx_adminshop:retrieveJobVehicles', function (source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('esx_adminshop:setJobVehicleState')
AddEventHandler('esx_adminshop:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('esx_adminshop: %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
