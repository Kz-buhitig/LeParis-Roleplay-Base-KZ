...
function OpenCustomersMenu()
	ESX.TriggerServerCallback('esx_banquier:getCustomers', function(customers)
		local elements = {
			...
			if data.value == 'deposit' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'customer_deposit_amount', {
					...
					if amount == nil then
						...
						menu2.close()
						TriggerServerEvent('esx_banquier:customerDeposit', customer.source, amount)
						OpenCustomersMenu()
					end
			...
			elseif data.value == 'withdraw' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'customer_withdraw_amount', {
					...
					if amount == nil then
						...
						menu2.close()
						TriggerServerEvent('esx_banquier:customerWithdraw', customer.source, amount)
						OpenCustomersMenu()
					end
				end, function(data2, menu2)
					menu2.close()
...
AddEventHandler('esx_banquier:hasEnteredMarker', function (zone)
	if zone == 'BankActions' and ESX.PlayerData.job and ESX.PlayerData.job.name == 'banquier' then
		CurrentAction     = 'bank_actions_menu'
		CurrentActionMsg  = _U('press_input_context_to_open_menu')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_banquier:hasExitedMarker', function (zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'banquier' then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_banquier:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_banquier:hasExitedMarker', lastZone)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'banquier' then
