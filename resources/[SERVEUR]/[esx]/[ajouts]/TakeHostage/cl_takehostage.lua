-----------------------------------------------------------------
--otage by Robbster, do not redistrbute without permission--
------------------------------------------------------------------
...
local otageAnimNamePlaying = ""
local otageAnimDictPlaying = ""
local otageControlFlagPlaying = 0
...
AddEventHandler('cmg3_animations:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
	...
	otageAnimNamePlaying = animation2
	otageAnimDictPlaying = animationLib
	otageControlFlagPlaying = controlFlag
end)
...
AddEventHandler('cmg3_animations:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
	...
	otageAnimNamePlaying = animation
	otageAnimDictPlaying = animationLib
	otageControlFlagPlaying = controlFlag
...
Citizen.CreateThread(function()
	while true do
		if holdingHostage or beingHeldHostage then 
			while not IsEntityPlayingAnim(GetPlayerPed(-1), otageAnimDictPlaying, otageAnimNamePlaying, 3) do
				TaskPlayAnim(GetPlayerPed(-1), otageAnimDictPlaying, otageAnimNamePlaying, 8.0, -8.0, 100000, otageControlFlagPlaying, 0, false, false, false)
				Citizen.Wait(0)
			end
		end
		Wait(0)
